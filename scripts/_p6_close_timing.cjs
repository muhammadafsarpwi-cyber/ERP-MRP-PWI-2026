/** Measure Save-click -> PATCH response -> Edit Location title removed from DOM. */
...
(async () => {
  ...
  let patchStart = null, patchDone = null, titleGoneAt = null;
  const t0 = Date.now();

  page.on('response', (resp) => {
    if (resp.request().method() === 'PATCH' && resp.url().includes('/warehouse-locations/')) {
      patchDone = Date.now() - t0;
      console.log('PATCH response at', patchDone, 'ms; HTTP', resp.status());
    }
  });
  ...
  if (await confirm.count()) {
    ...
    patchStart = tc;
    console.log('Save clicked at', tc, 'ms');
  }

  const deadline = Date.now() + 10000;
  while (Date.now() < deadline) {
    const open = (await page.locator('.ant-modal-title:has-text("Edit Location")').count()) > 0;
    if (!open) { titleGoneAt = Date.now() - t0; break; }
    await page.waitForTimeout(50);
  }
  console.log('title removed at', titleGoneAt, 'ms');
  console.log('click->response:', patchDone && patchStart ? patchDone - patchStart : 'n/a', 'ms');
  console.log('response->title gone:', titleGoneAt && patchDone ? titleGoneAt - patchDone : 'n/a', 'ms');
  console.log('click->title gone (harness budget ~1900-2400ms):', titleGoneAt && patchStart ? titleGoneAt - patchStart : 'n/a', 'ms');

  // restore: clear parent back to null via UI
  if (titleGoneAt) {
    await page.locator(`tr:has(td:text-is("${C01}")) button[title="Edit Location"]`).click();
    await page.waitForSelector('.ant-modal-title:has-text("Edit Location")', { timeout: 15000 });
    await page.waitForTimeout(700);
