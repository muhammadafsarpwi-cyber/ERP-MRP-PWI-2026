 ...
 *
 * The page provides its context (title + subtitle + icon + extra actions) via setHeaderMeta —
 * this feeds the single main header (breadcrumb → title → subtitle + actions) so pages no
...
export const useHeaderActions = create<HeaderActionsState>((set) => ({
  ...
  setHeaderMeta: (title, subtitle, icon, extra) => set({ title, subtitle, icon, extra }),
  clearHeaderTitle: () => set({ title: undefined, icon: undefined }),
  clearHeaderMeta: () => set({ title: undefined, subtitle: undefined, icon: undefined, extra: undefined }),
}));
