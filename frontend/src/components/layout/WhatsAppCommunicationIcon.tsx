...
interface SummaryResponse {
  success: boolean;
  data: { whatsapp: WhatsAppSummary };
}

const POLL_INTERVAL_MS = 30000;

const WhatsAppCommunicationIcon: React.FC = () => {
  ...
  const panel = (
    <div data-testid="whatsapp-panel" style={{
      width: 340,
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderBottom: '1px solid var(--theme-border)' }}>
        <Space size={8}>
          <MessageOutlined style={{ color: 'var(--theme-text)' }} />
          <span style={{ fontWeight: 600, color: 'var(--theme-text)' }}>WhatsApp Communication</span>
        </Space>
        <Space size={0}>
          <Button type="link" size="small" onClick={() => { setOpen(false); navigate('/communication/whatsapp-logs'); }} data-testid="whatsapp-open-logs">
            Logs
          </Button>
          <Button type="link" size="small" onClick={() => { setOpen(false); navigate('/communication/whatsapp-settings'); }} data-testid="whatsapp-open-settings">
            Settings
          </Button>
        </Space>
      </div>
      <div style={{ padding: 12 }}>
        {!summary?.configured ? (
          <div style={{ padding: 8, textAlign: 'center', color: 'var(--theme-text-muted)' }}>
            ...
            <div>WhatsApp delivery is not configured</div>
          </div>
        ) : (
        ...
        {loading && recent.length === 0 ? (
          ...
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No WhatsApp deliveries yet" />
        ) : (
            ...
            <List.Item style={{ borderBottom: '1px solid var(--theme-border)', padding: '6px 0' }}>
              <div style={{ minWidth: 0, width: '100%' }}>
                <div style={{ fontSize: 12, color: 'var(--theme-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {r.renderedSubject || r.recipientAddress || 'WhatsApp'}
                </div>
                <Space size={8}>
  ...
  return (
    <Dropdown trigger={['click']} open={open} onOpenChange={handleOpenChange} popupRender={() => panel}>
      <Tooltip title="WhatsApp communication">
        <span
...
export default WhatsAppCommunicationIcon;