...
// TASK 14: fully dynamic production flow. Stages are generated by the backend
// from the ACTUAL item chain (1 RAW MATERIAL stage + a PROCESS/OUTPUT pair per
// real downstream item). No fixed six-stage structure, no hardcoded FLATTENING
// / SPIRAL stages, no keyword-derived titles.
...
export const StageBlock: React.FC<StageBlockProps> = ({ stage }) => {
    ...
    return (
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6, flexWrap: 'wrap' }}>
          ...
          >
            STEP {String(stage.sequence).padStart(2, '0')}
          </span>
          <span
            style={{
              fontFamily: 'monospace',
          ...
          >
            ITEM {String(stage.itemNumber ?? stage.sequence).padStart(2, '0')}
          </span>
  ...
  return (
    ...
    >
      ...
      {isProcess ? (
        <>
          {/* Process block — operation emphasis (real data only) */}
          <Tooltip title={`${stage.departmentName ?? 'No department'} · ${stage.operationName ?? 'No operation'}`}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--theme-text, inherit)' }}>
                {stage.operationName || stage.departmentName || '—'}
              </span>
            </div>
          </Tooltip>
          {stage.operationCode && (
          ...
          {(stage.divisionName || stage.sectionName) && (
            <div style={{ fontSize: 10, color: 'var(--theme-text-muted, #94a3b8)', marginTop: 2 }}>
              {[stage.divisionName, stage.sectionName].filter(Boolean).join(' · ')}
            </div>
          )}
...
const ProductionFlowCard: React.FC<ProductionFlowCardProps> = ({ itemId, style }) => {
  ...
  const { stages, current, currentOperation, warning } = flow;
    ...
    <Card
      ...
      title={
        <Space>
          <ApartmentOutlined style={{ color: 'var(--theme-accent, #0284c7)' }} />
          <span style={{ fontWeight: 600 }}>Production Flow</span>
          <Tag color="purple" style={{ margin: 0 }}>01 → {String(stages.length).padStart(2, '0')}</Tag>
        </Space>
      }
      style={{ borderRadius: 8, ...style }}
    >
      {/* TASK 14: Cycle detection warning */}
      {warning && (
        <div
          style={{
            marginBottom: 10,
            padding: '6px 10px',
            borderRadius: 6,
            background: 'var(--theme-warning-soft, rgba(234, 179, 8, 0.12))',
            border: '1px solid var(--theme-warning, rgba(234, 179, 8, 0.35))',
            fontSize: 12,
            fontWeight: 600,
            color: 'var(--theme-warning, #ca8a04)',
          }}
        >
          {warning}
        </div>
      ...
      >
        ...
        <Tag color="cyan" style={{ margin: 0 }}>Operation: {currentOperation || '—'}</Tag>
        <Tag color="green" style={{ margin: 0 }}>
          {configuredCount} / {stages.length} stages configured
        </Tag>
      </div>

      {/* TASK 14: Dynamic flow — generated from actual item chain */}
      <div
        style={{
          display: 'flex',
          alignItems: 'stretch',
