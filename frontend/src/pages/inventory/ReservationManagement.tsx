...
const ReservationManagement: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingItem) {
        await apiService.patch(`/inventory/reservations/${editingItem.id}`, values);
        message.success('Reservation updated');
      } else {
        await apiService.post('/inventory/reservations', values);
  ...
  return (
    <Card title="Reservations">
      <Space style={{ marginBottom: 16 }} wrap>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={handleCreate}>New Reservation</Button>
      </Space>
