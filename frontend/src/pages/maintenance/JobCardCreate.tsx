...
import { BuildOutlined, ScanOutlined } from '@ant-design/icons';
...
export const JobCardCreate: React.FC = () => {
  ...
  const [form] = Form.useForm(); const [categoryForm] = Form.useForm(); const [loading, setLoading] = useState(false); const [lookupCode, setLookupCode] = useState(''); const [lookupLoading, setLookupLoading] = useState(false); const [selectedMachine, setSelectedMachine] = useState<JobCard | null>(null); const [categories, setCategories] = useState<OrgOption[]>([]); const [rootCategories, setRootCategories] = useState<OrgOption[]>([]); const [failureCategories, setFailureCategories] = useState<OrgOption[]>([]); const [categoryError, setCategoryError] = useState(''); const [relatedCategoryError, setRelatedCategoryError] = useState(''); const [categoryModalOpen, setCategoryModalOpen] = useState(false); const [categorySaving, setCategorySaving] = useState(false); const [org, setOrg] = useState<{ companies: OrgOption[]; divisions: OrgOption[]; sections: OrgOption[]; departments: OrgOption[] }>({ companies: [], divisions: [], sections: [], departments: [] });
  ...
  const lookupMachine = async () => { if (!lookupCode.trim()) return; setLookupLoading(true); try { const machine = await apiService.get<JobCard>(`/machines/by-code/${encodeURIComponent(lookupCode.trim())}`); if ((companyId && machine.companyId !== companyId) || (divisionId && machine.divisionId !== divisionId) || (sectionId && machine.sectionId !== sectionId) || (departmentId && machine.departmentId !== departmentId)) throw new Error('Machine is outside the selected organizational context'); setSelectedMachine(machine); form.setFieldValue('machineId', machine.id); message.success(`Machine found: ${machine.machineName || machine.name || machine.machineCode || machine.id}`); } catch (e) { setSelectedMachine(null); message.error(errorText(e)); } finally { setLookupLoading(false); } };
  ...
  return <div><PageHeader icon={<BuildOutlined />} title="Create Job Card" subtitle="Register a maintenance request" gradient="linear-gradient(135deg, #1f6f78 0%, #2e8b8b 100%)" showBreadcrumbs /><Card><Form form={form} layout="vertical" onFinish={submit} initialValues={initialValues}>
    <Typography.Title level={5}>Organization &amp; Asset</Typography.Title>{context ? <Descriptions bordered size="small" column={{ xs: 1, md: 3 }} style={{ marginBottom: 16 }}><Descriptions.Item label="Company">{context.companyName}</Descriptions.Item><Descriptions.Item label="Division">{context.divisionName}</Descriptions.Item><Descriptions.Item label="Section">{context.sectionName}</Descriptions.Item><Descriptions.Item label="Department">{context.departmentName}</Descriptions.Item><Descriptions.Item label="Machine">{context.machineName}{context.machineCode ? ` (${context.machineCode})` : ''}</Descriptions.Item></Descriptions> : <><Row gutter={16}><Col xs={24} md={6}><Form.Item name="companyId" label="Company" rules={[{ required: true, message: 'Company is required' }]}><Select showSearch optionFilterProp="label" options={org.companies.map(v => ({ value: v.id, label: optionLabel(v) }))} /></Form.Item></Col><Col xs={24} md={6}><Form.Item name="divisionId" label="Division" rules={[{ required: true, message: 'Division is required' }]}><Select showSearch disabled={!companyId} options={org.divisions.map(v => ({ value: v.id, label: optionLabel(v) }))} onChange={() => { form.resetFields(['sectionId', 'assignedDepartmentId', 'machineId']); setSelectedMachine(null); }} /></Form.Item></Col><Col xs={24} md={6}><Form.Item name="sectionId" label="Section" rules={[{ required: true, message: 'Section is required' }]}><Select showSearch disabled={!divisionId} options={org.sections.map(v => ({ value: v.id, label: optionLabel(v) }))} onChange={() => { form.resetFields(['assignedDepartmentId', 'machineId']); setSelectedMachine(null); }} /></Form.Item></Col><Col xs={24} md={6}><Form.Item name="assignedDepartmentId" label="Department"><Select showSearch disabled={!sectionId} options={org.departments.map(v => ({ value: v.id, label: optionLabel(v) }))} onChange={() => { form.resetFields(['machineId']); setSelectedMachine(null); }} /></Form.Item></Col></Row></>}
    {context && <><Form.Item name="companyId" hidden><Input /></Form.Item><Form.Item name="divisionId" hidden><Input /></Form.Item><Form.Item name="sectionId" hidden><Input /></Form.Item><Form.Item name="assignedDepartmentId" hidden><Input /></Form.Item></>}
    <Form.Item name="machineId" hidden rules={[{ required: true, message: 'Machine is required' }]}><Input /></Form.Item>
    <Row gutter={16} align="bottom"><Col xs={24} md={16}><Form.Item label="Machine (Scan QR / Enter Code)"><Input placeholder="Scan QR code or enter machine code, then press Enter" value={lookupCode} onChange={e => setLookupCode(e.target.value)} onPressEnter={lookupMachine} suffix={<ScanOutlined style={{ color: '#1890ff' }} />} /></Form.Item></Col><Col xs={24} md={8}><Form.Item><Button type="primary" icon={<ScanOutlined />} loading={lookupLoading} onClick={lookupMachine} block>Lookup Machine</Button></Form.Item></Col></Row>
    {selectedMachine && <Alert type="success" showIcon message={`Machine: ${selectedMachine.machineName || selectedMachine.name || selectedMachine.machineCode || selectedMachine.id}`} description={<Space size="large"><span>Code: {selectedMachine.machineCode || '—'}</span><span>Type: {selectedMachine.machineType || '—'}</span><span>Location: {selectedMachine.location || '—'}</span></Space>} style={{ marginBottom: 16 }} closable onClose={() => { setSelectedMachine(null); form.setFieldValue('machineId', null); }} />}
