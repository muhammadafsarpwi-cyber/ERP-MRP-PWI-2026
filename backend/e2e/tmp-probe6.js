/* quick dump of KPI summary region */
...
class Cdp {
  ...
  async eval(e) { const r = await this.send('Runtime.evaluate', { expression: e, returnByValue: true, awaitPromise: true });
    if (r.exceptionDetails) return 'ERR ' + String(r.exceptionDetails.exception && r.exceptionDetails.exception.description || '').slice(0, 200); return r.result.value; }
  async waitFor(e, t = 15000) { const t0 = Date.now(); for (;;) { let v=false; try{v=await this.eval(e);}catch{} if(v) return true; if(Date.now()-t0>t) return false; await new Promise(r=>setTimeout(r,250)); } }
}
(async () => {
  ...
  const M = (await c.query(`SELECT id FROM machines WHERE machine_code='SP-02' LIMIT 1`)).rows[0] || (await c.query(`SELECT id FROM machines WHERE is_active=true AND machine_code ILIKE 'sp%' ORDER BY machine_code LIMIT 1`)).rows[0];
  ...
  await c.end();
  const { child, port } = launchBrowser(9351);
  ...
  console.log(await cdp.eval(`({
    titles: [...document.querySelectorAll('.ant-card-title')].map(t=>t.textContent),
    kpiCard: (()=>{const x=[...document.querySelectorAll('.ant-card')].find(x=>x.querySelector('.ant-card-title')&&x.querySelector('.ant-card-title').textContent==='KPI Summary'); return x?x.innerText.replace(/\\n/g,' | ').slice(0,300):null;})(),
    anyRejection: document.body.innerText.includes('Rejection %'),
  })`));
  child.kill();
