/* probe7: step-by-step InputNumber writes -> what does the form see */
...
(async () => {
  ...
  await c.end();
  const { child, port } = launchBrowser(9353);
  ...
  const snap = `(() => {
    ...
    return { dom: inp.value, cls: fi.querySelector('.ant-input-number').className.slice(0,80), pct: m?+m[1]:null };
  })()`;

  const setV = async (v) => { await cdp.eval(`(()=>{const fi=[...document.querySelectorAll('.ant-form-item')].find(f=>f.querySelector('label')&&f.querySelector('label').textContent.trim()==='Rejection / Scrap'); const inp=fi.querySelector('.ant-input-number-input'); (${SET_INPUT})(inp, ${JSON.stringify(v)}); return 1})()`); };

  console.log('initial:', JSON.stringify(await cdp.eval(snap)));
  await setV('100'); await new Promise(r=>setTimeout(r,600));
  console.log('after 100:', JSON.stringify(await cdp.eval(snap)));
  await setV('50'); await new Promise(r=>setTimeout(r,600));
  console.log('after 50 :', JSON.stringify(await cdp.eval(snap)));
  // try with focus+blur dance
  await cdp.eval(`(()=>{const fi=[...document.querySelectorAll('.ant-form-item')].find(f=>f.querySelector('label')&&f.querySelector('label').textContent.trim()==='Rejection / Scrap'); const inp=fi.querySelector('.ant-input-number-input'); inp.focus(); return 1})()`);
  await setV('25'); 
  await cdp.eval(`(()=>{const fi=[...document.querySelectorAll('.ant-form-item')].find(f=>f.querySelector('label')&&f.querySelector('label').textContent.trim()==='Rejection / Scrap'); const inp=fi.querySelector('.ant-input-number-input'); inp.dispatchEvent(new Event('blur')); inp.blur(); return 1})()`);
  await new Promise(r=>setTimeout(r,700));
  console.log('after 25+blur:', JSON.stringify(await cdp.eval(snap)));
  child.kill();
